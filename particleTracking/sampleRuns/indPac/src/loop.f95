...
!!---------------------------------------------------------------------------          
  ...
  !cwgl open for debugging purposes
  !nwgl=0
  !OPEN(11, FILE='/data/guppy/willlush/Mercator/WGL_DEBUGGING_w_dxdy.bin',access='stream')
  !cwgl end open
...
!           ss0=dble(int(ts,8))*tseas/dxyz or should ssp be in the call cross_time?
           ...
           !wgl debug output
!            if (nwgl.eq.0) then
!               print*,'WRITING OUT ONE WFLUX FILE, WGL'
!               print *,'I AM GETTING CRANKY',size(uflux)
!               write(11) uflux
! 999           Format(F7.2)
!               !write(11) vflux
!               nwgl=1
...
return
   ...
   CONTAINS
     ...
     subroutine errorCheck(teststr,errCode)
       ...
       !cwgl - relax error codes - keep from killing big runs
       strict = 0
       !cwgl end
