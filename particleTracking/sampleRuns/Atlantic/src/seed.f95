   ...
   SUBROUTINE seed (tt,ts)
    ...
    ! INTEGER                                  :: ib, jb, kb, ibm
     ...
     ! --------------------------------------------
      ...
      !cwgl
      !print *,'seedintsdelta',seedintsdelta
      !---------------------------------------
      !---releases particles every seedintsdelta days---
      !---------------------------------------
      if ((ints-intstart-1)/seedintsdelta .ne. real((ints-intstart-1))/seedintsdelta) return
      ! ---------------------------------------
            ...
            ijjLoop: DO jjt=1,ijt
               kkkLoop: DO jkt=1,ikt          
                  ...
                  CASE (4)   ! Spread evenly inside box
                     ...
                     !cwgl
                     !x1 = DBLE (ibm)  + DBLE(jjt)/(DBLE(ijt)+1.0d0)+(1.0d0/((DBLE(ijt)+1.0d0)*4.0d0))
                     x1 = DBLE (ibm)  + (DBLE(jjt) - 0.5d0)/DBLE(ijt)
                     !x1 = DBLE (ibm)  + DBLE(jjt)/(DBLE(ijt)+1.0d0) !double at edges ...
                     !y1 = DBLE (jb-1) + DBLE(jkt)/(DBLE(ikt)+1.0d0)+(1.0d0/((DBLE(ikt)+1.0d0)*4.0d0))
                     y1 = DBLE (jb-1) + (DBLE(jkt) - 0.5d0)/DBLE(ikt)
                     !y1 = DBLE (jb-1) + DBLE(jkt)/(DBLE(ikt)+1.0d0)
                     z1 = DBLE (kb-1) + 0.5d0
                     !cwgl
                  ...
                  CASE (6) !cwgl actually spread evenly within box
                     x1 = DBLE (ibm)  + DBLE(jjt)/(DBLE(ijt)+1.0d0)+(1.0d0/(2.0d0*DBLE(ijt)))
                     !x1 = DBLE (ibm)  + DBLE(jjt)/(DBLE(ijt)+1.0d0) !double at edges ...
                     y1 = DBLE (jb-1) + DBLE(jkt)/(DBLE(ikt)+1.0d0)+(1.0d0/(2.0d0*DBLE(ikt)))
                     !y1 = DBLE (jb-1) + DBLE(jkt)/(DBLE(ikt)+1.0d0)
                     z1 = DBLE (kb-1) + 0.5d0
                     !cwgl end
