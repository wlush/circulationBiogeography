...
module mod_write
  ...
  USE mod_time
  !cwgl
  USE mod_grid, only: subGridImin,subGridJmin
  !end cwgl
 ...
 ! USE mod_traj, only: ib,jb,kb
  ...
  !changed true to false below cwgl
  LOGICAL                                    :: outdirdate = .false.
...
!======================================================================================
!cwgl edits - all recl were originally 24    
#if defined binwrite
    !NOTE FOR BINWRITE - twrite is in seconds!!!
    !twrite used for all output files except outdata_run.bin
    open(unit=75 ,file=trim(fullWritePref)//'_out.bin', &  
         access='direct' ,form='unformatted' ,recl=24 ,status='replace')
    !cwgl - 24 to 28 recl - commented out to change access
    open(unit=76 ,file=trim(fullWritePref)//'_run.bin', &  
         access='direct' ,form='unformatted' ,recl=28 ,status='replace')
    !cwgl end
...
!#endif
   ...
   enddo
    ...
    case (10) !in
       ...
       !write(unit=78 ,rec=recPosIn) ntrac,ints-1,x14,y14,z14
       write(unit=78 ,rec=recPosIn) ntrac,ints-1,subgridimin+x14 &
            ,subgridjmin+y14,z14
       return
    case (11)
       if(  (kriva == 1 .and. nrj(4,ntrac)  ==  niter-1 ) .or. &
 ...
 !         call interp(ib,jb,kb,x1,y1,z1,temp2,salt2, dens2,2)
          ...
          ! ntrac,ints,x1,y1,z1,tt/tday,t0/tday,subvol
          !cwgl - added ,twrite/tday (eq. to tt/tday)
          !write(unit=76 ,rec=recPosRun) ntrac,twrite,x14,y14,z14 !original
          !write(unit=76 ,rec=recPosRun) ntrac,twrite,x14,y14,z14,twrite/tday
          !write(unit=76 ,rec=recPosRun) ntrac,ints-1,x14,y14,z14,twrite/tday
          write(unit=76 ,rec=recPosRun) ntrac,ints-1,subgridimin + x14 &
               ,subgridjmin+y14,z14,twrite/tday
          !cwgl
       end if
    !cwgl edits below - twrite to twrite/tday
    case (13)
       recPosKll = recPosKll + 1
       !write(unit=77 ,rec=recPosKll) ntrac,twrite,x14,y14,z14
       write(unit=77 ,rec=recPosKll) ntrac,twrite/tday,x14,y14,z14
    case (15)
       recPosRun = recPosRun + 1
       !write(unit=76 ,rec=recPosRun) ntrac,twrite,x14,y14,z14
       write(unit=76 ,rec=recPosRun) ntrac,twrite/tday,x14,y14,z14
    case (17) !out
       recPosOut = recPosOut + 1
       !write(unit=77 ,rec=recPosOut) ntrac,twrite,x14,y14,z14
       write(unit=77 ,rec=recPosOut) ntrac,twrite/tday,x14,y14,z14
    case (19) !end
       recPosOut = recPosOut + 1
       !write(unit=75 ,rec=recPosOut) ntrac,twrite,x14,y14,z14
       write(unit=75 ,rec=recPosOut) ntrac,twrite/tday,x14,y14,z14
    case (40) !error
       recPosErr=recPosErr + 1    
       !write(unit=79 ,rec=recPosErr) ntrac,twrite,x14,y14,z14
       write(unit=79 ,rec=recPosErr) ntrac,twrite/tday,x14,y14,z14
    !cwgl end
    case (99) !switch
